...
namespace OCLParser
{
    ...
    {
        public static Expression ParseOCLExpression(string aExpression)
        {
            ...
            if (vKind == Expressionkind.Undefined)
            {
                ...
                for (int I = 0; I < vExpr.Length; I++)
                {
                    ...
                    if (((c == '+') || (c == '-') || (c == '*') || (c == '/') || (c == '<') || (c == '>') || (c == '=') || (c == '&') || (c == '|')) && (vParenthesisDepth == 0) && (!vInString))
                    {
                    ...
                    if ((I == vExpr.Length - 2) && (!vParenthesisDepthReachedZero))
            ...
            switch (vKind)
            {
                case Expressionkind.Condition:
                    ...
                    (Result as ConditionalExpression).Expression = ParseOCLExpression(vConditionExpression);
                    (Result as ConditionalExpression).If = ParseOCLExpression(vIfExpression);
                    (Result as ConditionalExpression).Else = ParseOCLExpression(vElseExpression);

                    break;
                case Expressionkind.Wrapped:
                    Result = ParseOCLExpression(vExpr.Substring(1, vExpr.Length - 2));
                    break;
                case Expressionkind.Member:
                    ...
                    Expression vExpression = ParseOCLExpression(vExpr.Substring((vDotIndex == vEndIndex) ? vEndIndex + 1 : vEndIndex));
                ...
                case Expressionkind.NamedOperation:
                    ...
                    Expression vExpression1 = ParseOCLExpression(vExpr.Substring((vDotIndex1 == vEndIndex1) ? vEndIndex1 + 1 : vEndIndex1));
                ...
                case Expressionkind.ParamOperation:
                    ...
                    (Result as ParamOperationExpression).Parameter = ParseOCLExpression(vExpr.Substring(vParenthesisStartIndex + 1, vParenthesisEndIndex - vParenthesisStartIndex - 1));
                    Expression vExpression2 = ParseOCLExpression(vExpr.Substring((vDotIndex2 == vEndIndex2) ? vEndIndex2 + 1 : vEndIndex2));
                ...
                case Expressionkind.OperatorOperation:
                    ...
                    for (int I = 0; I < vExpr.Length; I++)
                    {
                        ...
                        if (((c == '+') || (c == '-') || (c == '*') || (c == '/') || (c == '<') || (c == '>') || (c == '=') || (c == '&') || (c == '|')) && (vParenthesisDepth == 0) && (!vInString))
                        {
                            if (vExpr[I + 1] != '>' && (vExpr[I - 1] != '-'))
                            {
                                (Result as OperatorOperationalExpression).Expressions.Add(ParseOCLExpression(vExpr.Substring(vPreviousOperatorIndex + 1, I - vPreviousOperatorIndex - 1)));
                                (Result as OperatorOperationalExpression).Operators.Add(c.ToString());
                                vPreviousOperatorIndex = I;
                            }
                        }
                    }
                    (Result as OperatorOperationalExpression).Expressions.Add(ParseOCLExpression(vExpr.Substring(vPreviousOperatorIndex + 1, vExpr.Length - vPreviousOperatorIndex - 1)));
                    break;
            }
            return Result;
        }
